...
| 按键 | 功能 | 说明 |
|------|------|------|
| **W / ↑** | 加速前进 | +0.02 m/s，15 次到满速 (0.3 m/s) |
| **S / ↓** | 倒车 / 减速 | -0.02 m/s |
| **A / ←** | 左转 | +3.4°，13 次到满舵 (45°) |
| **D / →** | 右转 | -3.4° |
| **Space** | 刹车 | 油门归零 |
...
**状态栏显示**（窗口底部每 0.5s 更新）：
```
[MANUAL] t=5.2s | v=0.29m/s(令+0.30) str=+3.4° | pos=(2.13,1.50) yaw=+15° | goal=18.3m
│          │           │        │            │                    │
模式       时间    实际车速  目标车速   转向角        当前位置          离目标距离
```
...
| 参数 | 值 | 说明 |
|------|-----|------|
| 最大车速 | **0.3 m/s** | 与实车 `max_velocity=0.26` 一致 |
| 速度步长 | 0.02 m/s | 15 次按键到满速 |
| 转向步长 | 3.4° | 13 次按键到满舵 |
| 最大偏航率 | 0.9 rad/s (52°/s) | 转弯自动限速 |

**为什么限制到 0.3 m/s**：阿克曼车在 45° 满舵时转弯半径仅 0.315 m，车速与偏航率成正比：
...
**转弯限速机制**：控制器根据转向角反推允许车速，使偏航率不超过 0.9 rad/s。
例如 45° 满舵时车速自动限制到 0.284 m/s，保证转弯平顺。

若需更快速度（如直线巡航测试），修改 `scripts/run_seb_naver_sim.py` 中
`KeyboardController.max_throttle`（模型硬上限 1.575 m/s）。
